...
COPY --from=build /app/target/rooms-ms-0.0.1.jar /app/rooms-ms-0.0.1.jar

EXPOSE 8080

# Wait the X-Ray daemon to be ready before start the application
CMD /usr/bin/xray -o -n sa-east-1 & \
    ...
    java -jar /app/rooms-ms-0.0.1.jar